...
public class SynchroHttpClient_Test1 {
	 // utf-8字符编码
...
///只要加载同步HttpClient的SynchroHttpClient_Test1类时自动加载下面的static块
    static {
    ...
    private static String sendHttpGet(HttpGet httpGet) {
        ...
        String responseContent = null;
        try {
            // 创建默认的httpClient实例.会到线程池查询可用的连接，没有就会新建
            httpClient = getHttpClient();
            // 配置请求信息
            ...
            try {
                // 释放资源
                if (response != null) {//释放这个连接，将连接放回到连接池
                    response.close();
                }
            } catch (IOException e) {
                e.printStackTrace();
